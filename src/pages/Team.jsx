  ...
  return (
    <div className="mb-24">
      ...
      <div className="grid grid-cols-1 gap-x-10 gap-y-14 sm:grid-cols-2 lg:grid-cols-4">
        {people.map((p, i) => ( <PersonCard key={p.name} p={p} i={i + offset} /> ))} </div> </div> ); } export default function Team() {
  useLenis();
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
        ...
        <div className="mx-auto max-w-[1400px] px-5 py-24 sm:px-10 sm:py-32">
          <div className="grid items-center gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(380px,0.9fr)] lg:gap-16">
            <div className="max-w-2xl">
              ...
              <div className="mt-8">
                <CTAButton cta={{ label: "Springboard Alumni", onClick: scrollToSpringboard }} />
              </div>
            </div>
